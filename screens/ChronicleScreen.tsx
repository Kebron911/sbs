...
const ChronicleScreen: React.FC = () => {
    ...
    return (
        <div className="space-y-8">
            <h1 className="font-display text-3xl md:text-4xl text-white">The Chronicle</h1>
            
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* Entry Form */}
                <Card>
                    <h2 className="font-display text-xl text-accent mb-2">New Chronicle Entry</h2>
                    <p className="text-sm text-text-secondary mb-4">
                        Record the events of your journey. Your AI companion will analyze your entry and award Wisdom for your insights.
                    </p>
                ...
                {/* History */}
                <Card>
                    <h2 className="font-display text-xl text-accent mb-4">Chronicle History</h2>
                    <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                        {chronicleEntries.length > 0 ? (
                           chronicleEntries.map(e => <ChronicleHistoryCard key={e.id} entry={e} />)
                        ) : (
                            <p className="text-center text-text-secondary py-8">Your chronicle is empty. Record your first entry to begin your legend.</p>
                        )}
                    </div>
                </Card>
            </div>
